...
export default function BookingInterface({ doctor }) {
  ...
  return (
    <div className="bg-zinc-950/30 backdrop-blur-2xl rounded-2xl border border-white/6 p-6 shadow-sm hover:border-brand-500/15 hover:shadow-[0_10px_30px_rgba(6,148,162,0.02)] transition duration-300 group">
        ...
        <form action={handleSubmit} className="mt-6 border-t border-white/5 pt-6 flex flex-col gap-4 animate-fade-in">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              min={minDateTime}
              required
              className="w-full bg-white/5 border border-white/8 text-white placeholder-zinc-500 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition cursor-pointer"
            />
          </div>
          ...
          <div className="flex gap-3 mt-2">
            <button
              type="submit"
              disabled={pending}
              className="flex-1 bg-brand-500/10 backdrop-blur-md border border-brand-400/20 text-white hover:bg-brand-500/20 hover:border-brand-400/35 shadow-[0_4px_12px_rgba(6,148,162,0.05)] active:scale-[0.98] disabled:opacity-50 font-semibold py-2.5 text-xs transition duration-200 cursor-pointer rounded-lg"
            >

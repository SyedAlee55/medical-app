...
export default function PatientOnboardingForm({ hasError }) {
  ...
  return (
    <div className="relative overflow-hidden bg-zinc-900/60 border border-white/10 backdrop-blur-2xl rounded-2xl p-8 w-full max-w-lg shadow-[0_10px_50px_rgba(0,0,0,0.3)]">
      ...
      <form action={handleSubmit} className="flex flex-col gap-5">
        ...
        <button
          type="submit"
          disabled={isPending}
          className="w-full bg-brand-500/10 backdrop-blur-md border border-brand-400/20 text-white hover:bg-brand-500/20 hover:border-brand-400/35 font-semibold rounded-xl py-3 text-base shadow-[0_4px_15px_rgba(6,148,162,0.05)] active:scale-[0.98] transition-all duration-300 cursor-pointer mt-2 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {isPending && <Loader2 className="w-5 h-5 animate-spin" />}
          {isPending ? 'Saving profile...' : 'Finish Setup & Enter Dashboard'}
        </button>
      </form>
    </div>
  )
